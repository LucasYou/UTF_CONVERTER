...
from_utf8_to_utf16le(int infile, int outfile)
{
  ...
  while((bytes_read = read_to_bigendian(infile, &utf8_buf.bytes[0], 1)) > 0) {
    ...
    int temp = remaining_bytes;
    for(int i = 1; i <= temp; i++)
      bytes_read = read_to_bigendian(infile, &utf8_buf.bytes[i], 1);
...
from_utf8_to_utf16be(int infile, int outfile)
{
  ...
  while((bytes_read = read_to_bigendian(infile, &utf8_buf.bytes[0], 1)) > 0) {
    ...
    //utf16_glyph_t te = code_point_to_utf16be_glyph(code_point, &size_of_glyph);
    //printf("%.4x ", utf16_buf.upper_bytes);
...
utf8_one_byte_encode(code_point_t code_point)
{
  ...
  ret.bytes[0].byte = (uint32_t)code_point;
  return ret;
}
...
utf8_two_byte_encode(code_point_t code_point)
{
  ...
  ret.bytes[1].byte = ret.bytes[1].byte | buf;

  /* upper byte with '110x xxxx' */
  code_point >>= 6;
  buf = 0xC0 | (code_point & 0x1F);
  ret.bytes[0].byte = ret.bytes[0].byte | buf;
...
{
  /*&utf8_???_byte_decode() --> utf8_???_byte_decode*/
  switch(size) {
  case 1:
...
utf8_three_byte_decode(utf8_glyph_t glyph)
{
  ...
  //printf("%d\n", glyph.bytes[2].top_one.remaining);
